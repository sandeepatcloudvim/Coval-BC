report 50001 "Standard Sales Invoice Coval"
{
    RDLCLayout = './StandardSalesInvoiceCBR.rdl';
    ...
    dataset
    {
        dataitem(Header; "Sales Invoice Header")
        {
            ...
            dataitem(Line; "Sales Invoice Line")
            {
                ...
                column(LotNo_1_Line; LotNo[1])
                {
                }
                column(LotNo_2_Line; LotNo[2])
                {
                }
                column(LotNo_3_Line; LotNo[3])
                {
                }
                column(LotNo_4_Line; LotNo[4])
                {
                }
                column(LotNo_5_Line; LotNo[5])
                {
                }
                column(Qty_1_Line; Qty[1])
                {

                }
                column(Qty_2_Line; Qty[2])
                {

                }
                column(Qty_3_Line; Qty[3])
                {

                }
                column(Qty_4_Line; Qty[4])
                {

                }
                column(Qty_5_Line; Qty[5])
                {
                ...
                begin
                    ...
                    Clear(Qty);
                    recValueEntry.RESET;
                    recValueEntry.SETRANGE("Document No.", Line."Document No.");
                    recValueEntry.SETRANGE("Document Line No.", Line."Line No.");
                    recValueEntry.SetCurrentKey("Item Ledger Entry No.");
                    IF recValueEntry.FINDSET THEN
                        REPEAT
                            IF recValueEntry."Item Ledger Entry No." <> xEntryNo THEN BEGIN
                                IF recItemLedgerEntry.GET(recValueEntry."Item Ledger Entry No.") THEN BEGIN
                                    i := i + 1;
                                    LotNo[i] := recItemLedgerEntry."Lot No.";
                                    Qty[i] := ABS(recItemLedgerEntry.Quantity);
                                END;
                                xEntryNo := recValueEntry."Item Ledger Entry No.";
                            END;
            ...
            begin
                ...
                recCLE.SetRange("Document No.", "No.");
